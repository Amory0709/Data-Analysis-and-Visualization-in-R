...
stopifnot(all.equal(y,c(0.0, 0.5, 1.0) ))
...
extremes = function(x){
  meanM <- mean(x)
  sdS <- sd(x)
  return(x > (meanM+2*sdS) | x < (meanM-2*sdS))
}
...
#the even indexes.
xySplit = function(p){
    return(matrix(data = p, nrow = length(p)/2, ncol = 2, byrow = T))
}
...
cutWithGreatestMedianPrice = function(df){
   df_new <- aggregate(df["price"], by = df["cut"], FUN = median)# aggregate para must be list
    return(subset(df_new,df_new$price ==  max(df_new$price))$cut)
}
